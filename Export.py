        ...
        for m in r.matches:
            ...
            if m.Game != None:
                ...
                ws.cell(row=rownr, column=7,
                        value=sum(p.weight for p in GetPlayerByPairingNumber(m.p1nr).playerprefs if p.Game == m.Game))
                ws.cell(row=rownr, column=8,
                        value=sum(p.weight for p in GetPlayerByPairingNumber(m.p2nr).playerprefs if p.Game == m.Game))
            rownr += 1
            matchnr += 1
