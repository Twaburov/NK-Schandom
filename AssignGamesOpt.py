...
from Objects import GetAllMatches, Game, GetCombinedPreference, GetPlayerWeightForGame, GetPlayer1, GetAllPlayers, \
    Round, GetMatches, GetPlayer2
from pyomo.environ import Objective, Var, ConstraintList
...
def AssignGamesOpt():
    ...
    # Objective Function
    model.obj = Objective(expr=sum(model.x[m, g] * GetCombinedPreference(GetPlayerWeightForGame(GetPlayer1(m), g),
                                                                         GetPlayerWeightForGame(GetPlayer2(m), g))
    ...
    for p in all_players:
        for g in all_games:
            model.game_once_per_player.add(
                sum(model.x[m, g] for m in all_matches if (GetPlayer1(m) == p or GetPlayer2(m) == p)) <= 1.0)
